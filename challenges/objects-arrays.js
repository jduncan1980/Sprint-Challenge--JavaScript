...
function namesToLowerCase() {
	return zooAnimals.map((animal) => {
		return animal.animal_name.toLowerCase();
	});
}
const lowCaseAnimalNames = namesToLowerCase();
...
const lowPopulationAnimals = getLowPop();
...
function totalAnimals() {
	return zooAnimals.reduce((total, cur) => {
		return total + cur.population;
	}, 0);
}

const populationTotal = totalAnimals();
