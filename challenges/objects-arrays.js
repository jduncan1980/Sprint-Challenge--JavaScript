...
function listUniversities(grads) {
	return grads.map((grad) => {
		return grad.university;
	});
}

const universities = listUniversities(graduates);
...
function allContact(grads) {
	return grads.map((grad) => {
		return `${grad.first_name} ${grad.email}`;
	});
}

const contactInfo = allContact(graduates);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */

function findUnis(grads) {
	return grads.filter((grad) => grad.university.includes('Uni'));
}

const unisWithUni = findUnis(graduates);
...
function namesToLowerCase(animals) {
	return animals.map((animal) => {
		return animal.animal_name.toLowerCase();
	});
}
const lowCaseAnimalNames = namesToLowerCase(zooAnimals);
...
*/
function getLowPop(animals) {
	return animals.filter((animal) => animal.population < 5);
}

const lowPopulationAnimals = getLowPop(zooAnimals);
...
*/
function totalAnimals(animals) {
	return animals.reduce((total, cur) => {
		return total + cur.population;
	}, 0);
}

const populationTotal = totalAnimals(zooAnimals);
